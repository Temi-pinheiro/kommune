...
import { ViewJobReq } from '@/actions'
import { useModal } from '@/providers'

export const JobRequest = ({ jobReq }: { jobReq: JobReq }) => {
  const open = useModal().open

  const handleClick = () => {
    // Logic to open modal with jobReq details
    open(<ViewJobReq jobReq={jobReq} />)
  }
  return (
    <button
      onClick={handleClick}
    ...
    >
      ...
      <div className="mt-3 flex items-end justify-between max-lg:w-full">
        ...
        <span className="rounded-full h-6 min-xl:h-8 w-6 min-xl:w-8 overflow-clip bg-[#688E47]">
          {jobReq.owner.avatar ? (
            <img
